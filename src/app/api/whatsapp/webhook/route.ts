...
export async function POST(req: NextRequest) {
  try {
    ...
    if (textoOriginal.length > 5) {
      ...
      const promptSecretaria = `Você é a Secretária IA do VideoOps respondendo via WhatsApp.
...
export async function GET() {
  return NextResponse.json({ ok: true, webhook: "VideoOps WhatsApp Secretária IA ativa" })
}
