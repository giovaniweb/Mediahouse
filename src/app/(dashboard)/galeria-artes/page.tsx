...
export default function GaleriaArtesPage() {
  ...
  const qs = new URLSearchParams({ area: "design", limit: "48" })
  if (search) qs.set("search", search)
  const { data, isLoading } = useSWR<{ videos: Arte[]; total: number }>(`/api/publico/galeria?${qs}`, fetcher)
  const artes = data?.videos ?? []

  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-5">
        <h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2"><ImageIcon className="w-6 h-6 text-purple-400" /> Galeria de Artes</h1>
        <p className="text-sm text-zinc-500 mt-0.5">{data?.total ?? 0} artes finalizadas e aprovadas.</p>
      </div>

      <div className="relative mb-5 max-w-md">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar arte…"
